...
class KumbiaActiveRecord
{
    ...
    protected $_at = array();    
    /**
     * Campos que terminan en _in_user
     *
     * @var array
     */
    protected $_in_user = array();
    /**
     * Campos que terminan en _at_user
     *
     * @var array
     */
    protected $_at_user = array();
    /**
    ...
    protected function _dump_info($table, $schema = '')
    {
        ...
        foreach (self::get_meta_data($table) as $field) {
            ...
            ////////////////////////// INICIO AÑADIO JULIO MURILLO
            if (substr($field['Field'], strlen($field['Field']) - 8, 8) == '_at_user') {
                $this->_at_user[] = $field['Field'];
                $aliasAux = substr($field['Field'], 0, -8);
            }
            if (substr($field['Field'], strlen($field['Field']) - 8, 8) == '_in_user') {
                $this->_in_user[] = $field['Field'];
                $aliasAux = substr($field['Field'], 0, -8);
            }
            ///////////////////////// FIN AÑADIO JULIO MURILLO            
    ...
    public function save($values=null)
    {
        ...
        foreach ($this->not_null as $f) {
            ...
            if (!isset($this->$f) || is_null($this->$f) || $this->$f == '') {
                ...
                if (!$ex && in_array($f, $this->_at_user)) {
                    continue;
                }
                if ($ex && in_array($f, $this->_in_user)) {
                    continue;
                }               
                Flash::error("Error: El campo $f no puede ser nulo");
                return false;
        ...
        if ($ex) {
            ...
            foreach ($this->non_primary as $np) {
                ...
                ///////////////////////////////
                if (in_array($np, $this->_in_user)) {                    
                        $this->$np = Session::get('login');                   
                }
                //////////////////////////////
                if (isset($this->$np)) {
                    $fields[] = $np;
            ...
            foreach ($this->fields as $field) {
                if ($field != $this->primary_key[0] || $this->{$this->primary_key[0]}) {
                    ...
                    /////////////////////////////
                    if (in_array($field, $this->_at_user)) {
                        if ($config['type'] == 'oracle') {
                            $this->$field = Session::get('login');
                        } else {
                            $this->$field = Session::get('login');
                        }
                    }
                    if (in_array($field, $this->_in_user)) {
                        unset($this->$field);
                        //$this->$field = Session::get('login');
                    }
                    /////////////////////////////

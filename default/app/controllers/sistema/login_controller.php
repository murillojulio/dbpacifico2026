...
class LoginController extends BackendController {
    ...
    public function recover_password()
    {
        ...
        Logger::debug("Recuperar contraseña");
        if (Input::hasPost('usuario')) {
            if (Usuario::recoverPassword(Input::post('usuario'))) 
            {
                 Flash::valid('Hemos enviado una nueva contraseña a tu correo electrónico.');
                 return Redirect::toAction('entrar/');                
            }
            else {
                 Flash::error('Los datos ingresados no coinciden con ninguna cuenta en el sistema.');
            }            
        }        
    }

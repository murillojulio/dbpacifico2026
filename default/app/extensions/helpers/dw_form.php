...
class DwForm extends Form {
    ...
    protected static function _getAttrsClass($attrs, $type) {
        if($type==='form' OR $type==='form-multipart') {
            ...
            if($type == 'upload') {
				...
				else if($attrs['data-files'] == 'doc') {
                    $attrs['data-files'] = '/(\.|\/)(doc|docx|pdf|jpe?g|png)$/i';
                }
    ...
    /**
     * Método para crear un select multiple a partir de un array de objetos de ActiveRecord. <br />
     * Permite mostrar varios valores por fila y valor con slug
     *
     * @param string $field Nombre del select: modelo.campo
     * @param string, array $show Campo a mostrar de la consulta.  Es posible mostrar mas de un campo con array('campo1', 'campo2')
    ...
    public static function dbSelectMultiple($field, $show=null, $data=null, $blank='Seleccione', $attrs=null, $value=null, $label='', $help='') {
        ...
        $input.=  "<select id=\"$id\" name=\"$name\" $attrs multiple>$options</select>";
        if(self::$_help_block) {
            $input.= self::help($help);
        }
        $input.= self::getControls();
     ...
     * @return string
     */
    public static function dbSelectMultipleNoSeleccion($field, $show=null, $data=null, $attrs=null, $value=null, $label='', $help='') {
     ...
     * @return string
     */
    public static function upload_V2($field, $attrs=null, $label='', $id_label='', $id_icono='') {
        //Tomo los nuevos atributos definidos en las clases
        $attrs = self::_getAttrsClass($attrs, 'upload');
        //Armo el input
        $input = self::getControls();
        ...
        //Verifico si tiene un label
        $label = "<span id=$id_label>$label</span>";
        return '<div class="form-group btn btn-success fileinput-button"><i id='.$id_icono.' class="fa fa-upload fa-pd-expand"></i>'.$label.$input.'</div>'.PHP_EOL;
    ...
    public static function sendUpdate($title='Actualizar registro', $icon='fa-save', $attrs=array(), $text='actualizar') {
        return DwButton::save($title, $icon, $attrs, $text);
    }
    
     /**
     * Método para mostrar el botón para enviar un formulario tipo continuar
     * 
     * @param string $title
     * @param string $icon
     * @param array $attrs
    ...
    public static function dbCheck($field, $show = NULL, $data = NULL, $attrs = NULL, $value = NULL) {
        ...
        foreach ($data as $p) {
            ...
            if ($i == 1) {
                $checks .= '<div class="row">' . PHP_EOL;
            }

            $checks .= '<div class="col-md-3">' . PHP_EOL;
            $id = $field;
            $idx = $id . '_' . $p->$pk;
            $namex = $id . '[]';
     ...
     * @return string
     */
    public static function dbCheckId($field, $show = NULL, $data = NULL, $attrs = NULL, $value = NULL, $columnas = 4, $ancho_columnas='3' ) {
        if (is_array($attrs)) {
            $attrs = Tag::getAttrs($attrs);
        }
        ...
        foreach ($data as $p) {
            ...
            $checks .= '<div class="col-md-'.$ancho_columnas.'">' . PHP_EOL;
            $id = $field;
            //$idx = $id . '_' . $p->$pk;
            $idx = $p->$show;
            $namex = $id . '[]';
     ...
     /**
     * Crea campos radio que toma los valores de un array de objetos
     *
     * @param string $field Nombre de campo
     * @param string $show Campo que se mostrara (opcional)
     * @param array $data Array('modelo','metodo','param') (opcional)
     ...
     * @return string
     */
    public static function dbRadioId($field, $show = NULL, $data = NULL, $attrs = NULL, $value = NULL, $columnas = 1, $ancho_columnas='12' ) {
        if (is_array($attrs)) {
            $attrs = Tag::getAttrs($attrs);
        }
        ...
        foreach ($data as $p) {
            ...
            //$namex = $id . '[]';
            $namex = $id;
            $checks .= "<label class=\"radio\"><input id=\"$idx\" name=\"$namex\" type=\"radio\" value=\"{$p->$pk}\"";
            // Si es array $value para select multiple se seleccionan todos
            //var_dump($value); die();
            $checked = '';
            if (is_array($value)) {

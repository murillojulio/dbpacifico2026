...
class DwUpload {
    ...
    /**
     * Indica si cambia el nombre por un md5 mas el nombre del login
     */
    public $encryptNameWithLogin = FALSE;
    ...
    /**
     * Método para definir si utiliza un md5 mas el login como nombre
     * @param boolean $encrypt
     */
    public function setEncryptNameWithLogin($encrypt) {        
        $this->encryptNameWithLogin = $encrypt;
    }
     ...
     */
    public function setSize($size='20 MB', $width=0, $height=0, $removeOld=FALSE) {
        $this->maxSize = $size;
        if($width >0 && $height > 0) {
            $this->resize = TRUE;
    ...
    protected function _setFileName($rename) {        
        ...
        if($this->encryptNameWithLogin) {
            $login = Session::get('login');
            $name = $login.md5(uniqid().time()).'.'.$this->file_ext;
            $this->oldName = $_FILES[$this->file]['name'];
        }else {

...
Load::models('sistema/estado_usuario', 'sistema/perfil', 'sistema/recurso', 'sistema/recurso_perfil', 'sistema/acceso', 'util/correo_recover_password');
...
class Usuario extends ActiveRecord {
            ...
            if(DwAuth::isLogged()) {
                ...
                if(DwForm::isValidToken()) { //Si el formulario es válido
                    ...
                    if(DwAuth::login(array('login'=>$user), array('password'=>$pass), $mode)) {                        
                        ...
                        $estado_usuario->getEstadoUsuario($usuario->id);
                        if( $usuario->perfil_id != Perfil::SUPER_USUARIO && ($estado_usuario->estado_usuario != EstadoUsuario::ACTIVO) ) { 
                            DwAuth::setError('Lo sentimos pero tu cuenta se encuentra inactiva. <br />Si esta información es incorrecta contacta al administrador del sistema.');
                            Flash::error(DwAuth::getError());
                            DwAuth::logout();                            
                            return false;
                        }                         
    ...
    public function getListadoUsuario($estado, $order='', $page=0) {
        $columns = 'usuario.*, perfil.perfil, estado_usuario.estado_usuario, estado_usuario.descripcion, departamento.nombre AS departamento';
        $join = self::getInnerEstado();
        $join.= 'INNER JOIN perfil ON perfil.id = usuario.perfil_id ';        
        $join.= 'INNER JOIN departamento ON departamento.id = usuario.departamento_id';
        $conditions = "usuario.perfil_id != ".Perfil::SUPER_USUARIO;//Por el super usuario
    ...
    public function getInformacionUsuario($usuario) {
        ...
        $columnas = 'usuario.*, perfil.perfil, estado_usuario.estado_usuario, estado_usuario.descripcion, departamento.nombre AS departamento';
        $join = self::getInnerEstado();
        $join.= 'INNER JOIN perfil ON perfil.id = usuario.perfil_id ';        
        $join.= 'INNER JOIN departamento ON departamento.id = usuario.departamento_id ';
    ...
    public static function recoverPassword($data) {
        //Verifico que hayan datos del password
        $login = $data['login'];
        $email = $data['email'];
        
        $obj = new Usuario();
        $obj->find_by_sql("SELECT usuario.* FROM usuario WHERE usuario.login = '$login' AND usuario.email = '$email'");
        if($obj->id)
        {
            $new_password = CorreoRecoverPassword::generarClave(6);
            $correo = $obj->email;
            $person = $obj->nombre.' '.$obj->apellido;
            $body = '<td style="padding:10px">
            <p>Este mensaje te ha llegado porque has realizado una solicitud para recuperar tu 
                contraseña de acceso a la base de datos del <span class="il">Observatorio Pacífico y Territorio</span>.</p>
            <p>Tu nueva contraseña generada es:</p>
            ...
            $correo_enviado = CorreoRecoverPassword::send($correo, $person, $body);
            if($correo_enviado)
            {
                //$obj->fecha_actualizacion_pass = date('Y-m-d H:i:s');
        ...
        {                  
         ...
         /**
            * Devuelve un array "correo"=>"nombre" con los usuarios que le llegan las alertas
            * 
            * @return array $personas
            */
        public static function usuariosAlertar()
        {
            $obj_usuario = new Usuario();
            $usuarios = $obj_usuario->find("alerta='SI'");

            $personas = array();            
            foreach ($usuarios as $usuario)
            {
                  $personas[$usuario->email] = "'$usuario->nombre'";
            }
            return $personas;
        }
